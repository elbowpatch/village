...
DROP POLICY IF EXISTS "Authors can delete posts" ON posts;
...
DROP POLICY IF EXISTS "Chatrooms are public" ON chatrooms;
DROP POLICY IF EXISTS "Auth users can create rooms" ON chatrooms;
DROP POLICY IF EXISTS "CR messages are public" ON chatroom_messages;
DROP POLICY IF EXISTS "Auth users can send CR messages" ON chatroom_messages;
...
DROP POLICY IF EXISTS "DM participants can view" ON direct_messages;
DROP POLICY IF EXISTS "Auth users can send DMs" ON direct_messages;
DROP POLICY IF EXISTS "Artworks are public" ON artworks;
DROP POLICY IF EXISTS "Auth users can list artworks" ON artworks;
DROP POLICY IF EXISTS "Artists can update artworks" ON artworks;
DROP POLICY IF EXISTS "Artwork likes are public" ON artwork_likes;
DROP POLICY IF EXISTS "Auth users can like artworks" ON artwork_likes;
DROP POLICY IF EXISTS "Users can unlike artworks" ON artwork_likes;
DROP POLICY IF EXISTS "Follows are public" ON follows;
DROP POLICY IF EXISTS "Auth users can follow" ON follows;
DROP POLICY IF EXISTS "Users can unfollow" ON follows;
DROP POLICY IF EXISTS "News is public" ON news_articles;
DROP POLICY IF EXISTS "Users can view own saves" ON saved_items;
...
do $$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'news_articles_url_key'
  ) then
...
-- Add photo_url to chatrooms
alter table chatrooms add column if not exists photo_url text;

-- Chatroom members table (for join/leave and real counts)
...
DROP POLICY IF EXISTS "Members are public" ON chatroom_members;
DROP POLICY IF EXISTS "Auth users can join" ON chatroom_members;
DROP POLICY IF EXISTS "Users can leave" ON chatroom_members;
...
-- Delete all default seeded chatrooms so only user-created rooms exist
delete from chatrooms where created_by is null;

-- Add quote/edit support to comments
...
-- ── POST SLUGS (sequential numbering) ─────────────────────
-- Add slug column as auto-incrementing serial
create sequence if not exists posts_slug_seq start 1;

alter table posts add column if not exists slug integer unique;

-- Backfill existing posts with sequential slugs ordered by created_at
do $$
declare
  r record;
  n integer := 1;
begin
  for r in select id from posts order by created_at asc loop
    update posts set slug = n where id = r.id and slug is null;
    n := n + 1;
  end loop;
  -- Sync the sequence to the max used value
  perform setval('posts_slug_seq', coalesce(max(slug), 0) + 1, false) from posts;
end $$;

-- Trigger: auto-assign slug on new post insert
