...
function HomeFeed({ user, profile, onAuthRequired, onPlayRadio, onPlayTV, currentRadio, currentTV, onCreatePost, dataSaver = false, onToast }: {
  ...
  // Listen for post created by current user (guaranteed, no realtime dependency)
  useEffect(() => {
    function onNewPost(e: Event) {
      const post = (e as CustomEvent).detail?.post as Post
      if (!post) return
      if (feedTabRef.current === 'top') {
        setPosts(prev => prev.some(p => p.id === post.id) ? prev : [post, ...prev])
      }
    }
    window.addEventListener('new-post-created', onNewPost)
    return () => window.removeEventListener('new-post-created', onNewPost)
  }, [])

  // Realtime — posts from other users (best-effort, requires Supabase Realtime enabled)
  useEffect(() => {
...
function CreatePostModal({ user, profile, onClose }: { user: any; profile: Profile | null; onClose: () => void }) {
  ...
  async function publish() {
    ...
    const { data: inserted } = await supabase
      .from('posts')
      .insert({ author_id: user.id, content: text.trim(), media_url: mediaUrl, media_urls: mediaUrls })
      .select('*, profiles(*)')
      .single()
    setSubmitting(false)
    if (inserted) {
      window.dispatchEvent(new CustomEvent('new-post-created', { detail: { post: inserted } }))
    }
    onClose()
  }
