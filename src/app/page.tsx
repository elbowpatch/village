    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {/* Village logo — raised with drop shadow, no background */}
          <div style={{ position: 'relative', marginTop: -4 }}>
            <svg width="42" height="42" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg"
              style={{ filter: 'drop-shadow(0 3px 8px rgba(0,0,0,0.22)) drop-shadow(0 1px 3px rgba(0,0,0,0.14))', display: 'block' }}>
              {/* Orange house */}
...
function ShowbizTab() {
  ...
  // Use multiple free RSS feeds from public entertainment sources
  const RSS_FEEDS = [
    { url: 'https://feeds.bbci.co.uk/news/entertainment_and_arts/rss.xml', source: 'BBC Entertainment', country: 'gb', cat: 'celebrity' },
    { url: 'https://rss.nytimes.com/services/xml/rss/nyt/Movies.xml', source: 'NYT Movies', country: 'us', cat: 'movies' },
    { url: 'https://rss.nytimes.com/services/xml/rss/nyt/Music.xml', source: 'NYT Music', country: 'us', cat: 'music' },
    { url: 'https://api.allorigins.win/raw?url=https://variety.com/v/film/feed/', source: 'Variety Film', country: 'us', cat: 'movies' },
    { url: 'https://api.allorigins.win/raw?url=https://pitchfork.com/rss/news/feed/r.xml', source: 'Pitchfork', country: 'us', cat: 'music' },
  ]
  ...
  async function fetchShowbiz() {
    ...
    try {
      ...
      // Build a GNews-compatible URL using the free public API
      // Falls back to multiple RSS proxy calls
      const countryQ = selectedCountry?.query || ''
      const categoryKw = selectedCategory?.kw || 'entertainment'
      ...
      const parsed: ShowbizItem[] = channelItems.slice(0, 30).map((item, idx) => {
        ...
        // Use a curated set of placeholder images based on category
        const catImages: Record<string, string[]> = {
          ...
          awards: [
            ...
            'https://images.unsplash.com/photo-1618944847828-82e943c3bdb7?w=400&q=80',
          ],
        }
        const pool = catImages[category] || catImages.celebrity
        ...
        return {
          ...
          url: link,
          image: mediaContent || enclosure || fallback,
          source,
          publishedAt: pubDate,
...
function PostCard({ post, liked, saved, onLike, onSave, user, profile, onAuthRequired }: {
  ...
  async function handleShare() {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        if (!blob) return
        const file = new File([blob], `village-post-${postSlug}.png`, { type: 'image/png' })
        const shareData: ShareData = {
          title: `${p?.display_name || 'Village'} on Village`,
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          ...
          a.download = `village-post-${postSlug}.png`
          a.click()
        }
  ...
  useEffect(() => {
    const channel = supabase.channel(`post-stats-${post.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'post_likes', filter: `post_id=eq.${post.id}` }, () => {
        supabase.from('posts').select('likes_count,comments_count,saves_count').eq('id', post.id).single().then(({ data }) => {
          if (data) { setLiveLikes(data.likes_count); setCommentCount(data.comments_count); setLiveSaves(data.saves_count || 0) }
        })
      })
  ...
  return (
    <div ref={postCardRef} className="card" style={{
      ...
      {/* Action bar — X/Twitter style with counts */}
      <div style={{ display: 'flex', alignItems: 'center', padding: '4px 8px 6px', borderTop: '0.5px solid var(--border)', marginTop: 8, gap: 0 }}>
        ...
        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color = liked ? '#f4212e' : 'var(--text3)'; el.style.background = 'transparent' }}>
          ...
          {(liveLikes || 0) > 0 && <span style={{ fontSize: '0.78rem', fontWeight: 600, minWidth: 14 }}>{liveLikes}</span>}
        </button>
        {/* Comment */}
        ...
        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color = expanded ? 'var(--accent)' : 'var(--text3)'; el.style.background = 'transparent' }}>
          ...
          {(commentCount || 0) > 0 && <span style={{ fontSize: '0.78rem', fontWeight: 600, minWidth: 14 }}>{commentCount}</span>}
        </button>
        {/* Repost/Share */}
        ...
        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color = 'var(--text3)'; el.style.background = 'transparent' }}>
          ...
          {(post.reposts_count || 0) > 0 && <span style={{ fontSize: '0.78rem', fontWeight: 600, minWidth: 14 }}>{post.reposts_count}</span>}
        </button>
        {/* Save/Bookmark */}
        ...
        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color = saved ? '#1d9bf0' : 'var(--text3)'; el.style.background = 'transparent' }}>
          ...
          {(liveSaves || 0) > 0 && <span style={{ fontSize: '0.78rem', fontWeight: 600, minWidth: 14 }}>{liveSaves}</span>}
        </button>
      </div>
      ...
      {shareCapturing && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontWeight: 800, fontSize: '0.85rem', letterSpacing: '-0.3px', color: 'var(--text)' }}>
              Village<span style={{ color: 'var(--accent)' }}>.</span>
            </span>
          </div>
          <span style={{ fontSize: '0.7rem', color: 'var(--text3)', fontWeight: 500 }}>
            village.app/post/{postSlug}
          </span>
        </div>
      )}
...
function MessagesPage({ user, profile }: { user: any; profile: Profile | null }) {
  ...
  if (!user) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 20px', gap: 12 }}>
      ...
      <div style={{ fontSize: '0.85rem', color: 'var(--text3)' }}>Connect privately with anyone on Village</div>
    </div>
  )
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {room.photo_url ? <img src={room.photo_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : (iconMap[room.icon] || '💬')}
        </div>
        <div style={{ flex: 1 }}>
