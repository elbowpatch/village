    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <img src="/chitchat-logo.png" alt="ChitChat" className="glitch-logo" style={{ width: 36, height: 36, objectFit: 'contain', display: 'block' }} />
          <span style={{ fontWeight: 900, fontSize: '1.25rem', fontFamily: "'Dancing Script', 'Pacifico', cursive" }}>
            <span style={{ color: 'var(--logo-blue)' }}>chit</span><span style={{ color: 'var(--logo-orange)' }}>chat</span>
          </span>
        </div>
...
function PostCard({ post, liked, saved, onLike, onSave, user, profile, onAuthRequired, dataSaver = false, onToast }: {
  ...
  function handleSave() {
    if (!user) { onAuthRequired(); return }
    onSave()
    fireToast(saved ? 'Removed from saved' : '🔖 Saved!', 'bookmark')
  }
  ...
  async function handleShare() {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        if (!blob) return
        const file = new File([blob], `chitchat-post-${postSlug}.png`, { type: 'image/png' })
        const shareData: ShareData = {
          title: `${p?.display_name || 'ChitChat'} on ChitChat`,
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          ...
          a.download = `chitchat-post-${postSlug}.png`
          a.click()
  ...
  return (
    <div ref={postCardRef} id={`post-${post.id}`} className="friendster-card" style={{
      position: 'relative',
      ...(shareCapturing ? { borderRadius: 20, boxShadow: '0 8px 32px rgba(0,0,0,0.18)' } : {})
    }}>
      {/* Card toast feedback */}
      {cardToast && (
        <div style={{
          position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)',
          background: 'rgba(0,0,0,0.82)', color: '#fff', borderRadius: 99,
          padding: '6px 16px', fontSize: '0.82rem', fontWeight: 600, zIndex: 20,
        ...
        <div style={{
          ...
          {post.media_urls.map((url, i) => (
            <div key={i} style={{ aspectRatio: '1/1', overflow: 'hidden' }}>
              <img src={dataSaver ? (url + '?w=200&q=40') : url} alt={`Photo ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', filter: dataSaver ? 'none' : undefined }} loading="lazy" />
            </div>
          ))}
        </div>
      ) : post.media_url ? (
        <div style={{ overflow: 'hidden', lineHeight: 0 }}>
          <img src={dataSaver ? (post.media_url + '?w=400&q=40') : post.media_url} alt="Post image" style={{ width: '100%', maxHeight: dataSaver ? 300 : 500, objectFit: 'cover', display: 'block' }} loading="lazy" />
        </div>
      ) : null}
      ...
      {shareCapturing && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontWeight: 800, fontSize: '0.88rem', letterSpacing: '-0.3px', fontFamily: "'Dancing Script','Pacifico',cursive" }}>
              <span style={{ color: 'var(--logo-blue)' }}>chit</span><span style={{ color: 'var(--logo-orange)' }}>chat</span>
            </span>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '0.62rem', color: '#999', fontWeight: 500, marginBottom: 1 }}>shared from</div>
            <div style={{ fontSize: '0.72rem', color: 'var(--logo-blue)', fontWeight: 700 }}>chitchat.app/post/{postSlug}</div>
          </div>
        </div>
      )}
