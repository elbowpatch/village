...
function PostCard({ post, liked, saved, onLike, onSave, user, profile, onAuthRequired, dataSaver = false, onToast }: {
  ...
  function handleLike() {
    if (!user) { onAuthRequired(); return }
    onLike()
    fireToast(liked ? 'Unliked' : '❤️ Liked!', 'heart')
  }
  ...
  function handleEcho() {
    ...
    setEchoActive(v => {
      const next = !v
      setLiveReposts(prev => next ? prev + 1 : Math.max(0, prev - 1))
      fireToast(next ? '🔁 Echoed!' : 'Echo removed', 'zap')
      return next
    })
  }
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          ...
          const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `cc-post-${postSlug}.png`; a.click()
  ...
  async function submitComment() {
    ...
    if (data) {
      ...
      if (post.author_id !== user.id) {
        await supabase.from('notifications').insert({ user_id: post.author_id, actor_id: user.id, type: 'comment', post_id: post.id, comment_id: data.id })
      }
      if (parentId && parentAuthorId && parentAuthorId !== user.id && parentAuthorId !== post.author_id) {
        await supabase.from('notifications').insert({ user_id: parentAuthorId, actor_id: user.id, type: 'reply', post_id: post.id, comment_id: data.id })
      }
    }
  }
  ...
  return (
    <div ref={postCardRef} id={`post-${post.id}`} style={{
      ...
      fontFamily: "'Plus Jakarta Sans', sans-serif",
      position: 'relative',
      ...(shareCapturing ? { borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.18)' } : {})
    }}>
      {cardToast && (
        <div style={{ position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.82)', color: '#fff', borderRadius: 99, padding: '6px 16px', fontSize: '0.82rem', fontWeight: 600, zIndex: 20, whiteSpace: 'nowrap', pointerEvents: 'none', animation: 'fadeInUp 0.2s ease' }}>{cardToast}</div>
      )}

      <div style={{ display: 'flex' }}>
        {/* Reddit left vote column */}
        <div style={{ width: 40, background: 'var(--bg3)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0', gap: 2, flexShrink: 0 }}>
          <button onClick={handleLike} style={{ width: 24, height: 24, borderRadius: 4, background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: liked ? '#ff4500' : 'var(--text3)', transition: 'color 0.15s, background 0.15s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,69,0,0.1)'; (e.currentTarget as HTMLElement).style.color = '#ff4500' }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = liked ? '#ff4500' : 'var(--text3)' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill={liked ? '#ff4500' : 'none'} stroke={liked ? '#ff4500' : 'currentColor'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 15 12 9 6 15"/></svg>
          </button>
          <span style={{ fontSize: '0.72rem', fontWeight: 800, lineHeight: 1, color: liked ? '#ff4500' : 'var(--text)', minWidth: 24, textAlign: 'center' }}>
            {liveLikes > 999 ? `${(liveLikes / 1000).toFixed(1)}k` : liveLikes}
          </span>
          <button style={{ width: 24, height: 24, borderRadius: 4, background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', transition: 'background 0.15s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(113,128,150,0.12)' }}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {/* Post header */}
          <div style={{ padding: '8px 10px 4px', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <Avatar letter={p?.avatar_letter || 'U'} color={grad} size={20} photoUrl={p?.avatar_url} />
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text)' }}>{p?.display_name || 'Unknown'}</span>
            {p?.verified && <VerifiedBadge size="sm" />}
            {post.chatroom_tag && (
              <span style={{ background: 'var(--accent)', color: '#fff', borderRadius: 99, padding: '1px 7px', fontSize: '0.62rem', fontWeight: 700 }}>#{post.chatroom_tag}</span>
            )}
            <span style={{ fontSize: '0.7rem', color: 'var(--text3)', marginLeft: 2 }}>· {timeAgo(post.created_at)}</span>
            <div style={{ position: 'relative', marginLeft: 'auto' }} ref={menuRef}>
              ...
              {postMenu && (
                <div style={{ position: 'absolute', right: 0, top: 24, background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 8, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', zIndex: 99, minWidth: 150, overflow: 'hidden' }}>
                  {user?.id === post.author_id && (
                    <>
                      <button onClick={() => { setEditPostModal(true); setPostMenu(false) }} style={{ width: '100%', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text)', fontFamily: "'Plus Jakarta Sans',sans-serif" }}><Icon name="edit" size={13} color="var(--accent)" /> Edit</button>
                      <div style={{ height: '0.5px', background: 'var(--border)' }} />
                      <button onClick={deletePost} style={{ width: '100%', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: 8, color: 'var(--accent2)', fontFamily: "'Plus Jakarta Sans',sans-serif" }}><Icon name="x" size={13} color="var(--accent2)" /> Delete</button>
                    </>
                  )}
                  <button onClick={() => { navigator.clipboard?.writeText(localContent); setPostMenu(false) }} style={{ width: '100%', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text)', fontFamily: "'Plus Jakarta Sans',sans-serif" }}><Icon name="share" size={13} color="var(--text3)" /> Copy text</button>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Content + media */}
          <div style={{ padding: '0 10px 8px', cursor: 'pointer' }} onClick={toggleExpand}>
            {localContent && (
            ...
            {(post.media_urls && post.media_urls.length > 1) ? (
              <div style={{ display: 'grid', gridTemplateColumns: post.media_urls.length === 2 ? '1fr 1fr' : post.media_urls.length === 3 ? '1fr 1fr 1fr' : '1fr 1fr', gap: 2, overflow: 'hidden', borderRadius: 4, marginBottom: 4 }}>
                {post.media_urls.map((url, i) => (
                  <div key={i} style={{ aspectRatio: '1/1', overflow: 'hidden' }}>
                    <img src={dataSaver ? (url + '?w=200&q=40') : url} alt={`Photo ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} loading="lazy" />
                  </div>
                ))}
              </div>
            ) : post.media_url ? (
              <div style={{ overflow: 'hidden', borderRadius: 4, marginBottom: 4 }}>
          ...
          {/* Reddit action bar */}
          <div style={{ display: 'flex', alignItems: 'center', borderTop: '1px solid var(--border)', background: 'var(--bg3)', padding: '0 2px' }}>
            {/* Comments */}
            <button onClick={toggleExpand} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 10px', background: 'transparent', border: 'none', cursor: 'pointer', color: expanded ? 'var(--accent)' : 'var(--text3)', fontSize: '0.76rem', fontWeight: 700, borderRadius: 2, transition: 'background 0.15s, color 0.15s' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(0,0,0,0.06)'; (e.currentTarget as HTMLElement).style.color = 'var(--text)' }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = expanded ? 'var(--accent)' : 'var(--text3)' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
              <span>{commentCount} Comments</span>
            </button>

            {/* Echo (repost) */}
            <button onClick={handleEcho} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 10px', background: 'transparent', border: 'none', cursor: 'pointer', color: echoActive ? '#46d160' : 'var(--text3)', fontSize: '0.76rem', fontWeight: 700, borderRadius: 2, transition: 'background 0.15s, color 0.15s' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(0,0,0,0.06)'; (e.currentTarget as HTMLElement).style.color = '#46d160' }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = echoActive ? '#46d160' : 'var(--text3)' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill={echoActive ? '#46d160' : 'none'} stroke={echoActive ? '#46d160' : 'currentColor'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>
              <span>{liveReposts} Echo</span>
            </button>

            {/* Share */}
            <button onClick={handleShare} disabled={shareLoading} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 10px', background: 'transparent', border: 'none', cursor: shareLoading ? 'wait' : 'pointer', color: 'var(--text3)', fontSize: '0.76rem', fontWeight: 700, borderRadius: 2, transition: 'background 0.15s, color 0.15s', opacity: shareLoading ? 0.6 : 1 }}
              onMouseEnter={e => { if (!shareLoading) { (e.currentTarget as HTMLElement).style.background = 'rgba(0,0,0,0.06)'; (e.currentTarget as HTMLElement).style.color = 'var(--text)' }}}
              ...
              {shareLoading ? <Icon name="loader" size={15} color="currentColor" /> : <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>}
              <span>{liveShares} Share</span>
            </button>
          </div>

          {/* Share branding — screenshot only */}
          {shareCapturing && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderTop: '0.5px solid var(--border)', background: 'var(--surface)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <img src="/cc-logo.png" alt="CC" style={{ width: 22, height: 22, objectFit: 'contain' }} />
                <span style={{ fontWeight: 800, fontSize: '0.85rem' }}>CC</span>
              </div>
              <div style={{ fontSize: '0.68rem', color: 'var(--accent)', fontWeight: 700 }}>/post/{postSlug}</div>
            </div>
          )}

          {/* Inline expanded comments */}
          {expanded && (
            <div style={{ borderTop: '1px solid var(--border)', background: 'var(--bg)', padding: '12px 12px 12px 16px' }}>
              ...
              {comments.filter(c => !c.parent_comment_id).map(c => (
                <div key={c.id}>
                  ...
                  {comments.filter(r => r.parent_comment_id === c.id).map(reply => (
                    <div key={reply.id} style={{ marginLeft: 32, borderLeft: '2px solid var(--border)', paddingLeft: 10 }}>
              ...
              <div style={{ marginTop: 10 }}>
                {replyTo && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    ...
                    <button onClick={() => setReplyTo(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}><Icon name="x" size={11} color="var(--text3)" /></button>
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                  {profile && <Avatar letter={profile.avatar_letter || 'U'} color={profile.avatar_color || '135deg,#007aff,#5856d6'} size={28} photoUrl={profile.avatar_url} />}
                  <div style={{ flex: 1, background: 'var(--surface)', borderRadius: 4, border: '1px solid var(--border)', padding: '8px 12px', display: 'flex', gap: 8, alignItems: 'flex-end' }}>
