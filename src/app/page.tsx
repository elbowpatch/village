    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <img src="/glitch-logo.png" alt="Glitch" className="glitch-logo" style={{ width: 36, height: 36, objectFit: 'contain', display: 'block' }} />
          <span style={{ fontWeight: 900, fontSize: '1.2rem', letterSpacing: '-0.5px', color: 'var(--text)', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            glitch
          </span>
        </div>
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', gap: 24, alignItems: 'flex-start', padding: '0 0 0 0' }}>
      ...
      <div style={{ padding: '12px 16px' }}>
        ...
        {/* Posts — Tumblr-style feed */}
        {feedTab === 'top' && !loading && posts.length > 0 && (
          <div className="tumblr-feed">
            {posts.map(post => (
...
function PostCard({ post, liked, saved, onLike, onSave, user, profile, onAuthRequired }: {
  ...
  return (
    <div ref={postCardRef} className="tumblr-card" style={{
      marginBottom: 0, overflow: 'hidden',
      ...(shareCapturing ? { borderRadius: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.18)' } : {})
    }}>
      {/* Tumblr-style: Media FIRST (full bleed), then content + compact header */}
      {(post.media_urls && post.media_urls.length > 1) ? (
        ...
        <div style={{ overflow: 'hidden', lineHeight: 0 }}>
          <img src={post.media_url} alt="Post image" style={{ width: '100%', maxHeight: 500, objectFit: 'cover', display: 'block' }} />
        </div>
      ) : null}
      ...
      <div style={{ padding: '14px 16px 0' }}>
        {localContent && (
          <div onClick={toggleExpand} style={{ cursor: 'pointer', marginBottom: 10 }}>
            <p style={{ fontSize: '0.95rem', lineHeight: 1.65, color: 'var(--text)', fontWeight: 400 }}>
              <RichContent text={localContent} onLinkClick={url => setLinkPopupUrl(url)} />
            </p>
          </div>
        )}
        {/* Compact header below content — Tumblr style */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingBottom: 10, borderBottom: '0.5px solid var(--border)' }}>
          <Avatar letter={p?.avatar_letter || 'U'} color={grad} size={28} photoUrl={p?.avatar_url} />
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', minWidth: 0 }}>
            ...
            <span style={{ fontSize: '0.72rem', color: 'var(--text3)', marginLeft: 'auto' }}>{timeAgo(post.created_at)}</span>
          </div>
          {/* Post menu */}
          <div style={{ position: 'relative' }} ref={menuRef}>
            <button onClick={() => setPostMenu(v => !v)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>
              <Icon name="more-h" size={16} color="var(--text3)" />
            </button>
            {postMenu && (
      ...
      {shareCapturing && (
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '0.72rem', color: '#7c3aed', fontWeight: 700 }}>glitch.app/post/{postSlug}</div>
          </div>
        </div>
      )}
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        ...
        <div style={{ fontWeight: 800, fontSize: '1.3rem', marginBottom: 4 }}>
          {mode === 'login' ? 'Welcome back' : 'Join Glitch'}
        </div>
